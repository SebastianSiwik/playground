...
Tile(tileset, size, tilesetPositions.at(0), position), tilesetPositions(tilesetPositions), duration(duration), tileToDraw(0) {}
...
void AnimatedTile::draw(Graphics &graphics) {
	SDL_Rect destRect = { position.x, position.y, size.x * globals::SPRITE_SCALE, size.y * globals::SPRITE_SCALE };
	SDL_Rect sourceRect = { tilesetPositions.at(tileToDraw).x, tilesetPositions.at(tileToDraw).y, size.x, size.y };
